...
- **Core Systems**:
  ...
  - `Time trials` — chronos monument→monument générés côté client (déterministe `tt_<dest>_<poiA>_<poiB>`, A* 200m–3km) ; départ sur place via Entrée (pas de téléport), beacons 3D + minimap, state machine `idle/countdown/running/finished` dans `GameEngine`, table `trial_times`, server fns `src/server/trials.ts`, UI `src/ui/trials/`
