...
- **Core Systems**:
  ...
  - `Time trials` — chronos monument→monument générés côté client (déterministe `tt_<poiA>_<poiB>` sans destination, vol d'oiseau 200m–3km, A* comme preuve de faisabilité, liste par beacon via `getBeaconTrials`) ; départ sur place via Entrée (pas de téléport), beacons 3D + minimap, state machine `idle/countdown/running/finished` dans `GameEngine`, table `trial_times`, server fns `src/server/trials.ts`, UI `src/ui/trials/`
