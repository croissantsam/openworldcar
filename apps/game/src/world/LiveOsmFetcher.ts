...
function parseOsmXml(xmlText: string): {
  ...
  // ways carry no tags, so they were absent from the scene until now.
  ...
  for (const b of relationBuildings) buildings.push(b)
  ...
  fillMissingHeights(buildings)

  return { roads, buildings, waterways, parks, railways, barriers, taggedNodes, nodes, wayNodeRefs }
}
